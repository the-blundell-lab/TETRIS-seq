...
'''''
Watson code for Grouping Reads by UMI and SSCS calling to create files for subsequent translocation calling or FLT3-ITD calling, but just restricting to regions of interest (i.e. if rearrangement found in raw BAM file).
Differs from CNV/ SNV SSCS calling because it does not do any filtering on sam flags.
...
Input:
    ...
    7) list of chromosomal regions to target
...
Usage:
Watson_code_SSCS_calling_for_translocations_1.1.py  --infile mapped_merged_BAM --sample-name sample_name --min-family-size NUMBER(default = 1, range 1-no max)"
                                 --threshold NUMBER(default=0.9, range 0-1) --max_N (default=1.0, range: 0-1) --outbam SSCS_bam_filename
                                 --min-mapping-quality NUMBER(default=20) --regions 
...
#Group reads by UMI
def Group_reads_by_UMI(sample_name, merged_bam, out_directory, min_mapping_qual, min_family_size, regions):
    ...
    for (chrom, start, end) in regions:
        for line in in_bam.fetch(chrom, start, end):
            ...
            if line.is_unmapped is False: #check the read is mapped...
                ...
                ##### THIS BIT IS NEW COMPARED TO STANDARD SSCS CALLING ######################################
                try:
                ...
                read_info = (sequence, cigar, qname, quality, mapq, XT_tag) #### XT TAG HAS BEEN ADDED IN HERE #################################################
                key = str((UMI, chromosome, coordinate, read_pair, strand, flag, partner_chromosome, partner_coordinate, template_length)) #### SAME AS STANDARD SSCS CALLING
                ...
                ####### HAVE GOTTEN RID OF THE 'if flag in good_flags list...' #######################################################################
    ...
    metrics_file.write('produced from code:\t' + 'Watson_code_SSCS_calling_for_translocations: version '+str(version) + '\n')
    ...
    metrics_file.write('mapped_good_mapping_quality (>= '+str(min_mapping_qual)+')\t' + str(read1_forward_good_mapping_qual)+'\t' + str(read2_forward_good_mapping_qual)+'\t' + str(read2_forward_good_mapping_qual)+'\t' + str(read2_reverse_good_mapping_qual)+'\t' + '\n')
    metrics_file.close()
...
def consensus_read_caller_with_qualities(grouped_reads, threshold, read_length, quality_threshold): #function for calling the consensus sequence from the UMI family
    ...
    return consensus_read, quality_reads, len(grouped_reads), consensus_error_rate, consensus_depth, consensus_errors, max_read_depth, min_read_depth, prob_error
...
def SSCS_calling_BAM(sample_name, grouped_read_dict, merged_bam, threshold, min_family_size, max_N, SSCS_bam, unpaired_SSCS_bam, out_directory, quality_threshold, version, min_mapping_qual):
    ...
    new_PG = [{'ID': 'Watson_code_SSCS_calling_for_translocations_v'+str(version),
    'PN': 'Watson_code_CNV_panel',
    'VN': version,
    'CL': 'Watson_code_SSCS_calling_for_translocations_v'+str(version)+'.py --infile '+merged_bam+' --sample-name '+sample_name+' --min-family-size '+str(min_family_size)+\
    ...
    for k, v in grouped_read_dict.items(): #reads grouped by UMI, with the following structure...
        ...
        XT_tags = [] #a list where the XT tag positions are stored
        ...
        ######################################## THIS IS NEW IN THE FLT3/ TRANSLOCAITON CALLING #########################################################
        #find the most common XT tag position in the UMI family group (i.e. where the adapter clipping is)
        for i in v:
            XT_tag = i[5]
            XT_tags.append(XT_tag)
        ################################################################################################################################################################

        most_common_CIGAR = most_common(CIGAR_group) #if there are only 2 and they are different then it takes the first one
        most_common_XT = most_common(XT_tags) #if there are only 2 and they are different then it takes the first one ######################################## NEW ########################################
        ...
        if UMI_family_size>=min_family_size: #check to make sure there are enough reads in the family (i.e. more than whatever minimum size was set)...
            ...
            consensus_read, quality_read, family_size, consensus_error_rate, consensus_depth, consensus_errors, max_read_depth, min_read_depth, prob_error = consensus_read_caller_with_qualities(consensus_group, threshold, read_length, quality_threshold) #call the consensus from the reads
            ...
            if proportion_N < max_N: #if proportion of Ns in the consensus is less than the set limit:
                ...
                ######################################## NEW ################## MISSING STRAND AND FLAG FROM THE KEY ################## i.e. effectively just strand info
                key = (UMI, chromosome, coordinate, partner_chromosome, partner_coordinate, template_length, read_pair)
                ...
                consensus_groups[key]=(consensus, CIGAR, UMI_family_size, quality, cD_tag, cM_tag, cE_tag, cd_tag, ce_tag, QS_tag, flag, XT_tag, strand)
                ...
                partner_key = (UMI, partner_chromosome, partner_coordinate, chromosome, coordinate, partner_template_length, partner_read_pair)
                ...
                if XT_tag != '':
                    new_read.set_tag('XT', XT_tag) #################### new ####################
                ...
                if partner_key in consensus_groups.keys(): #i.e. if the reads are paired (has the partner survived the N filter and min family size filter?)
                    ...
                    partner_flag = partner[10]
                    partner_XT_tag = partner[11]
                    partner_strand = partner[12]
                    ...
                    if XT_tag != '':
                        partner_read.set_tag('XT', partner_XT_tag)
    ...
    if len(consensus_groups)>0: #if there are any remaining UMI families in the dictinary (these will just be the remaining unpaired reads (i.e. those not already added to BAM/ fastq...)
        for k, v in consensus_groups.items():
            ...
            chromosome = k[1]
            coordinate = k[2]
            partner_chromosome = k[3]
            partner_coordinate = k[4]
            template_length = k[5]
            read_pair = k[6]
            consensus = v[0]
            CIGAR = v[1]
...
def main():
    ...
    parser.add_argument("--regions", nargs='+', action="store", dest="regions", help="list of regions to target as chrom start end triples ", required=True)
    o = parser.parse_args()
    ...
    target_regions = o.regions

    regions = [(target_regions[i], int(target_regions[i+1]), int(target_regions[i+2])) 
            for i in range(0, len(target_regions), 3)]

    print('targeted regions = ', regions)

    #Group reads by UMI
    grouped_read_dict = Group_reads_by_UMI(sample_name, merged_bam, out_directory, min_mapping_qual, min_family_size, regions)
    #Call SSCS
    ...
    return print('SSCS calling for translocation calling complete')
