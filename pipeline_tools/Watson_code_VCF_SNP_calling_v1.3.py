...
Usage:
Watson_code_VCF_SNP_calling_1.3.py  --infile mpileup --sample-name sample_name --path-to-reference-genome path
                                    --dbSNP_shelve_dictionary path to dbSNP shelve dictionary
                                    --mean_base_qual_filter float --mapq_filter --read_pos_filter --bias_filter
...
def basics(chromosome, position, ID, ref, alt, qual, filt):
    return chromosome+'\t'+str(position)+'\t'+ID+'\t'+ref.upper()+'\t'+alt.upper()+'\t'+str(qual)+'\t'+filt

def info_section(sample_name, variant_type, depth, end, variant_depth, allele_frequency, UMI_ref, UMI_alt, ID, MAF_1000_genomes, ref_strand_bias, var_strand_bias, mean_pos_read, std_pos_read, mean_base_qual, std_base_qual, strand_fisher, FS, mean_mapq):
    return str('SAMPLE='+sample_name)+';TYPE='+str(variant_type)+';DP='+str(depth)+';END='+str(end)+';VD='+\
                ...
                ';PSTD='+str(round(std_pos_read))+';QUAL='+str(round(float(mean_base_qual), 2))+';QSTD='+str(round(float(std_base_qual), 2))+\
...
def text_output(chromosome, position, end_position, ID, MAF_1000_genomes, ref, alt, depth, variant_depth, allele_frequency, variant_type, UMI_ref, UMI_alt, ref_strand_bias, var_strand_bias, mean_pos_read, std_pos_read, mean_base_qual, std_base_qual, strand_fisher, FS, mean_mapq):
    return chromosome+'\t'+str(position)+'\t'+str(end_position)+'\t'+ref.upper()+'\t'+alt.upper()+'\t'+ID+'\t'+str(MAF_1000_genomes)+'\t'+\
            ...
            str(round(float(mean_base_qual)))+'\t'+str(round(float(std_base_qual)))+'\t'+str(abs(mean_mapq))+'\t'+\
            str(ref_strand_bias)+'\t'+str(var_strand_bias)+'\t'+str(round(strand_fisher, 5))+'\t'+str(FS)+'\n'

def position_analysis(reference, depth, bases, UMI_family_size, read_positions, quality, mapq):
...
#             print("bases[0] in ['.', ',']")
            ...
            if (len(bases) == 1) or (bases[1] not in ['+', '-']): #i.e. if it isn't the end of the string and isn't followed by insertion or deletion
                ...
                #add the quality score of that base to the quality score counter
                try:
                    quality_counter[reference].append(ord(quality[0])-33) #ord converts the ASCII to (decimal) number
                    try:
                        quality = quality[1:]
                    except IndexError:
                        print(quality)
            ...
            ##### INSERTION OR DELETION FOLLOWING A REFERENCE BASE ########
                ...
                if 'N' not in indel_sequence_record: #i.e. if the indel does not contain an N
                    ...
                    #add the quality score of the insertion to the quality score counter
                    if indel_sequence_record in quality_counter[indel_marker].keys():
                        quality_counter[indel_marker][indel_sequence_record].append(ord(quality[0])-33) #ord converts the ASCII to (decimal) number
                    else:
                        quality_counter[indel_marker][indel_sequence_record]=[ord(quality[0])-33]
                    quality = quality[1:]
...
#             print("bases[0] in ['*', '#']")
            if (len(bases) == 1) or (bases[1] not in ['+', '-']):
                ...
                #add the quality score of the deletion to the quality score counter
                quality_counter['*'].append(ord(quality[0])-33)
                quality = quality[1:]
            ...
            ##### INSERTION OR DELETION FOLLOWING A REFERENCE DELETION ######## - RECORD AS COMPLEX
                ...
                if 'N' not in indel_sequence_record: #i.e. if the indel does not contain an N
                    ...
                    #add the quality score of the insertion to the quality score counter
                    if indel_sequence_record in quality_counter['X'].keys():
                        quality_counter['X'][indel_sequence_record].append(ord(quality[0])-33) #ord converts the ASCII to (decimal) number
                    else:
                        quality_counter['X'][indel_sequence_record]=[ord(quality[0])-33]
                    quality = quality[1:]
...
#             print("bases[0] in ['<', '?']")
            if (len(bases) == 1) or (bases[1] not in ['+', '-']):
                ...
                #add the quality score of the deletion to the quality score counter
                quality_counter['<>'].append(ord(quality[0]))
                quality = quality[1:]
            ...
            if (len(bases) == 1) or (bases[1] not in ['-', '+']): #i.e. if alternate base and not followed by insertion of deletion
                ...
                read_positions = read_positions[1:]
                quality_counter[upper_base].append(ord(quality[0])-33)
                quality = quality[1:]
    ...
    for k, v in quality_counter.items():
        if k in ['A', 'G', 'C', 'T', '*', '<>']:
            if len(v) > 0:
                qualities_counter[k]=(np.mean(v), np.std(v))
            else:
                qualities_counter[k]=(0, 0)
        if k in ['+', '-', 'X']:
            if len(v)>0:
                new_v = {}
                for indel, qual in v.items():
                    new_v[indel]=(np.mean(qual), np.std(qual))
                qualities_counter[k]=new_v
            else:
                qualities_counter[k]=(0, 0)
...
#         print('recal_depth = ', recal_depth)
    ...
    return base_counter, strand_counter, UMI_counter, pos_in_read_counter, qualities_counter, mapping_qualities_counter, recal_depth

def VCF_calling(base_counter, strand_counter, UMI_counter, pos_in_read_counter, qualities_counter, mapping_qualities_counter, mean_base_qual_filter, mapq_filter, read_pos_filter, bias_filter, reference, chromosome, position, dbSNP_dict, depth, sample_name, VCF, TEXT):
    alt_positions = 0
    ...
    for i in bases: #iterate through each of the alternate bases so can create a VCF line for them
        ...
        #     strand_bias = str(strand_counter[i]['F'])+':'+str(strand_counter[i]['R']) ?WHAT IS THIS RATIO?
                ...
                mean_base_qual = qualities_counter[alt][0]
                std_base_qual = qualities_counter[alt][1]
                ...
                filt_list = []
                if mean_base_qual < mean_base_qual_filter:
                    filt_list.append('q'+str(mean_base_qual_filter))
                ...
                BASIC_INFO = basics(chromosome, position, ID, ref, alt, mean_base_qual, filt)
                INFO = info_section(sample_name, variant_type, depth, end, variant_depth, allele_frequency, UMI_ref, UMI_alt, ID, MAF_1000_genomes, ref_strand_bias, var_strand_bias, mean_pos_read, std_pos_read, mean_base_qual, std_base_qual, strand_fisher, FS, mean_mapq)
                ...
                TEXT_FILE_INFO = text_output(chromosome, position, end_position, ID, MAF_1000_genomes, ref, alt, depth, variant_depth, allele_frequency, variant_type, UMI_ref, UMI_alt, ref_strand_bias, var_strand_bias, mean_pos_read, std_pos_read, mean_base_qual, std_base_qual, strand_fisher, FS, mean_mapq)
    ...
    if len(base_counter['+']) > 0: #i.e. there is an insertion present
        ...
        #         filt_list.append('InIns')
            ...
            BASIC_INFO = basics(chromosome, position, ID, ref, ref+alt, mean_base_qual, filt) #record alt as ref+alt for an insertion
            INFO = info_section(sample_name, variant_type, depth, end, variant_depth, allele_frequency, UMI_ref, UMI_alt, ID, MAF_1000_genomes, ref_strand_bias, var_strand_bias, mean_pos_read, std_pos_read, mean_base_qual, std_base_qual, strand_fisher, FS, mean_mapq)
    #             INFO = info_section(sample_name, variant_type, depth, end, variant_depth, allele_frequency, ref_strand_bias, mean_base_qual, std_base_qual, strand_fisher, FS, mean_mapq, five_prime, three_prime)
            FORMAT = format_section(GT, DP, VD, AD, AF, RD, ALD)
            ...
            #Write the text file (which only contains positions with variants)
            end_position = position+(len(ref)-1)
            TEXT_FILE_INFO = text_output(chromosome, position, end_position, ID, MAF_1000_genomes, ref, ref+alt, depth, variant_depth, allele_frequency, variant_type, UMI_ref, UMI_alt, ref_strand_bias, var_strand_bias, mean_pos_read, std_pos_read, mean_base_qual, std_base_qual, strand_fisher, FS, mean_mapq)
            TEXT.write(TEXT_FILE_INFO)
    ...
    if len(base_counter['-']) > 0: #i.e. there is an insertion present
        ...
        #         filt_list.append('InIns')
            ...
            BASIC_INFO = basics(chromosome, position, ID, ref+alt, ref, mean_base_qual, filt)
            INFO = info_section(sample_name, variant_type, depth, end, variant_depth, allele_frequency, UMI_ref, UMI_alt, ID, MAF_1000_genomes, ref_strand_bias, var_strand_bias, mean_pos_read, std_pos_read, mean_base_qual, std_base_qual, strand_fisher, FS, mean_mapq)
    #             INFO = info_section(sample_name, variant_type, depth, end, variant_depth, allele_frequency, ref_strand_bias, mean_base_qual, std_base_qual, strand_fisher, FS, mean_mapq, five_prime, three_prime)
            FORMAT = format_section(GT, DP, VD, AD, AF, RD, ALD)
            ...
            TEXT_FILE_INFO = text_output(chromosome, position, end_position, ID, MAF_1000_genomes, ref+alt, ref, depth, variant_depth, allele_frequency, variant_type, UMI_ref, UMI_alt, ref_strand_bias, var_strand_bias, mean_pos_read, std_pos_read, mean_base_qual, std_base_qual, strand_fisher, FS, mean_mapq)
            TEXT.write(TEXT_FILE_INFO)
    ...
    if len(base_counter['X']) > 0: #i.e. there is a complex variant present
        ...
        for k, v in base_counter['X'].items(): #in case there is more than 1 insertion:
            ...
            #     strand_bias = str(strand_counter[i]['F'])+':'+str(strand_counter[i]['R']) ?WHAT IS THIS RATIO?
                ...
                mean_base_qual = qualities_counter['X'][alt][0]
                std_base_qual = qualities_counter['X'][alt][1]
                strand_fisher, FS = fisher_exact(strand_counter[ref]['F'], strand_counter[ref]['R'],
                ...
                BASIC_INFO = basics(chromosome, position, ID, ref, alt_sequence, mean_base_qual, filt)
                INFO = info_section(sample_name, variant_type, depth, end, variant_depth, allele_frequency, UMI_ref, UMI_alt, ID, MAF_1000_genomes, ref_strand_bias, var_strand_bias, mean_pos_read, std_pos_read, mean_base_qual, std_base_qual, strand_fisher, FS, mean_mapq)
        #             INFO = info_section(sample_name, variant_type, depth, end, variant_depth, allele_frequency, ref_strand_bias, mean_base_qual, std_base_qual, strand_fisher, FS, mean_mapq, five_prime, three_prime)
                FORMAT = format_section(GT, DP, VD, AD, AF, RD, ALD)
                ...
                #Write the text file (which only contains positions with variants)
                end_position = position+(len(ref)-1)
                TEXT_FILE_INFO = text_output(chromosome, position, end_position, ID, MAF_1000_genomes, ref, alt_sequence, depth, variant_depth, allele_frequency, variant_type, UMI_ref, UMI_alt, ref_strand_bias, var_strand_bias, mean_pos_read, std_pos_read, mean_base_qual, std_base_qual, strand_fisher, FS, mean_mapq)
                TEXT.write(TEXT_FILE_INFO)
    ...
    #     strand_bias = str(strand_counter[i]['F'])+':'+str(strand_counter[i]['R']) ?WHAT IS THIS RATIO?
        ...
        mean_base_qual = qualities_counter[ref][0]
        std_base_qual = qualities_counter[ref][1]
        strand_fisher = 0
        FS = 0
...
def main():
    ...
    parser.add_argument('--mean_base_qual_filter', type=float, default=22.5, dest='mean_base_qual_filter', help="mean base quality filter (no filtering, just highlights in filter column). [default = 22.5]")
    ...
    mean_base_qual_filter = o.mean_base_qual_filter
    ...
    with open(mpileup, 'r') as pileupfile:
        ...
        ####### CREATE TEXT FILE #######
        TEXT = open(out_directory+'/'+sample_name+'_watson_code_'+ECS_type+'_variant_calling_variants_and_SNPs_MUFS'+str(min_family_size)+'.txt', 'w')
        ...
        TEXT.write('##mean_position_in_read, std_position_in_read, mean_base_quality, std_base_quality, mean_mapq are all calculated from just the variant reads, except where only REF is present'+'\n')
        ...
        ####### CREATE VCF FILE #########
        VCF = open(out_directory+'/'+sample_name+'_watson_code_'+ECS_type+'_variant_calling_all_positions_MUFS'+str(min_family_size)+'.vcf', 'w')
        ...
        VCF.write('##INFO=<ID=UMI_ref,Number=1,Type=Float,Description="Mean UMI family size for ref">\n')
        VCF.write('##INFO=<ID=UMI_alt,Number=1,Type=Float,Description="Mean UMI family size for alt">\n')
        ...
        VCF.write('##FILTER=<ID=q'+str(mean_base_qual_filter)+',Description="Mean Base Quality Below '+str(mean_base_qual_filter)+'">\n')
        ...
        VCF.write('#CHROM\t'+'POS\t'+'ID\t'+'REF\t'+'ALT\t'+'QUAL\t'+'FILTER\t'+'INFO\t'+'FORMAT\t'+sample_name+'\n')
                ...
                if depth !=0:
                    ...
                    for i in bases:  #get rid of first position covered by read symbols ('^' and then it's assoc quality)
                        if i == '^':
                            ...
                            assoc_qual = bases[first_pos+1]
                            bases = bases.replace('^'+assoc_qual, '')
                    ...
                    UMI_family_size = []
                    for i in qname:
                        qname_list_split = i.split(':')
                        r1_UMI_family_size = int(qname_list_split[1])
                        r2_UMI_family_size = int(qname_list_split[2])
                        mean_UMI_family_size = (r1_UMI_family_size+r2_UMI_family_size)/2
                        UMI_family_size.append(mean_UMI_family_size)

                    position_details = position_analysis(reference, depth, bases, UMI_family_size, read_positions, quality, mapq)
                    ...
                    qualities_counter = position_details[4]
                    mapping_qualities_counter = position_details[5]
                    recal_depth = position_details[6]

                    VCF_calling(base_counter, strand_counter, UMI_counter, pos_in_read_counter, qualities_counter, mapping_qualities_counter,
                                   mean_base_qual_filter, mapq_filter, read_pos_filter, bias_filter, reference, chromosome, position, dbSNP_dict, recal_depth, sample_name, VCF, TEXT)
