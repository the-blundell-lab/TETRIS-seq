...
Usage:
Watson_code_VCF_SNP_calling_1.2.py  --infile mpileup --sample-name sample_name --path-to-reference-genome path
...
def VCF_calling(base_counter, strand_counter, UMI_counter, pos_in_read_counter, qualities_counter, mapping_qualities_counter, mean_base_qual_filter, mapq_filter, read_pos_filter, bias_filter, reference, chromosome, position, dbSNP_dict, depth, sample_name, VCF, TEXT):
    ...
    if len(base_counter['+']) > 0: #i.e. there is an insertion present
        ...
        #         filt_list.append('InIns')
            ...
            BASIC_INFO = basics(chromosome, position, ID, ref, ref+alt, mean_base_qual, filt)
            INFO = info_section(sample_name, variant_type, depth, end, variant_depth, allele_frequency, UMI_ref, UMI_alt, ID, MAF_1000_genomes, ref_strand_bias, var_strand_bias, mean_pos_read, std_pos_read, mean_base_qual, std_base_qual, strand_fisher, FS, mean_mapq)
    ...
    if len(base_counter['-']) > 0: #i.e. there is an insertion present
        for k, v in base_counter['-'].items(): #in case there is more than 1 insertion:
            ...
            RSID, MAF = dbSNP_information(chromosome, position, ref+alt, ref, dbSNP_dict)
            ...
            if row_count > 0:
                ...
                if depth !=0:
                    ...
                    VCF_calling(base_counter, strand_counter, UMI_counter, pos_in_read_counter, qualities_counter, mapping_qualities_counter,
                                   mean_base_qual_filter, mapq_filter, read_pos_filter, bias_filter, reference, chromosome, position, dbSNP_dict, depth, sample_name, VCF, TEXT)
