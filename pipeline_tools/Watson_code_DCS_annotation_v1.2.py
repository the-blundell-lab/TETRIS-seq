...
Input:
    1) DCS variants and SNPs text file
...
Usage:
Watson_code_DCS_annotation_v1.2.py  --variants_infile variants and SNPs text file --annotated_infile annovar annotated file
...
def create_annotated_df(annotated_file, gene_list, transcripts_list):
        ...
        for row in readreader:
            ...
            if row_count > 2:
                ...
                if gene in gene_list:
                            ...
                            if i in transcripts_list:
                                ...
                                if len(transcript)>4:
                                    try:
                                        ...
                                        protein_change = transcript[n+2].split(',')[0]
                                else:
                                    protein_change = '.'
                                break
                            n+=1
                    ...
                    DCS_ann_dict[(chromosome, start, end, ref, alt)]=(gene, transcript_info, intronic_exonic, exon, cDNA, protein_change, exonic_function,
    ...
    DCS_ann_df = pd.DataFrame.from_dict(DCS_ann_dict, orient = 'index', columns = ['gene', 'transcript', 'intronic_exonic', 'exon', 'cdNA', 'AA_change', 'exonic_function',
    ...
    DCS_ann_df = DCS_ann_df.reset_index()
...
def create_variants_df(variants_file):
        ...
        for row in readreader:
            if row_count > 5:
                ...
                if variant_depth >0:
                    ...
                    mean_mapq = float(row[15])
                    REF_depth_by_strand = row[16]
                    variant_depth_by_strand = row[17]
                    strand_bias_fisher_p_value = float(row[18])
                    strand_bias_fisher_p_value_phred = float(row[19])

                    DCS_variants_dict[((chromosome, start, end, ref, alt))] = (total_depth, variant_depth, VAF, variant_type, REF_mean_UMI_family_size,
    ...
    DCS_variants_df = pd.DataFrame.from_dict(DCS_variants_dict, orient = 'index', columns = ['total_depth', 'variant_depth', 'VAF', 'variant_type', 'REF_mean_UMI_family_size',
                                                                                            'variant_mean_UMI_family_size', 'mean_position_in_read', 'std_position_in_read',
                                                                                            'mean_mapq', 'REF_depth_by_strand', 'variant_depth_by_strand', 'strand_bias_fisher_p_value',
                                                                                            'strand_bias_fisher_p_value_phred', 'RSID', 'MAF_1000_genomes'])
    DCS_variants_df = DCS_variants_df.reset_index()

    return DCS_variants_df
...
def main():
    ...
    final_df = total_df[['chromosome', 'start', 'end', 'REF', 'ALT', 'total_depth', 'variant_depth', 'VAF',
           ...
           'mean_position_in_read', 'std_position_in_read', 'mean_mapq',
