...
'''''
Watson code for Grouping Reads by UMI and SSCS calling to create files for subsequent translocation calling.
...
Outputs:
    Fastq file:
        1)  Interleaved SSCS paired fastq file
    BAM files:
...
Watson_code_SSCS_calling_for_translocations_1.1.py  --infile mapped_merged_BAM --sample-name sample_name --min-family-size NUMBER(default = 1, range 1-no max)"
                                 ...
                                 --outfastq SSCS fastq filename --min-mapping-quality NUMBER(default=20)
...
def Group_reads_by_UMI(sample_name, merged_bam, out_directory, min_mapping_qual, min_family_size):
    ...
    for line in bam_entry:
        ...
        if line.is_unmapped is False: #check the read is mapped...
            ...
            read_info = (sequence, cigar, qname, quality, mapq, XT_tag)
...
def SSCS_calling_BAM(sample_name, grouped_read_dict, merged_bam, threshold, min_family_size, max_N, SSCS_bam, SSCS_fastq, unpaired_SSCS_bam, out_directory, quality_threshold, version, min_mapping_qual):
    ...
    ' --threshold '+str(threshold)+ ' --max_N '+str(max_N)+' --outbam '+ SSCS_bam + ' --outfastq '+ SSCS_fastq+\
    ...
    for k, v in grouped_read_dict.items(): #reads grouped by UMI, with the following structure...
        ...
        most_common_XT = most_common(XT_tags) #if there are only 2 and they are different then it takes the first one
                ...
                if partner_key in consensus_groups.keys(): #i.e. if the reads are paired (has the partner survived the N filter and min family size filter?)
                    ...
                    if read_pair == 'read1':
                        ...
                        if strand == 'forward':
                            ...
                            if partner_strand == 'forward': #it shouldn't be, but sometimes there might have been mismapping
                                partner_read.seq = partner_consensus
                                partner_read.query_qualities = partner_quality
                            fastq.write('@:%s\n%s\n+\n%s\n' % (partner_read.qname, partner_read.seq, partner_read.query_qualities))
                        ...
                        if strand == 'reverse':
                            ...
                            if partner_strand == 'forward':
                                partner_read.seq = partner_consensus
                                partner_read.query_qualities = partner_quality
                            if partner_strand == 'reverse': #it shouldn't be, but sometimes there might have been mismapping
                                sequence = Seq(partner_consensus)
                                partner_read.seq = str(sequence.reverse_complement())
                                partner_read.query_qualities = reverse(partner_quality)
                            fastq.write('@:%s\n%s\n+\n%s\n' % (partner_read.qname, partner_read.seq, partner_read.query_qualities))
                    ...
                    if read_pair == 'read2':
                        ...
                        if strand == 'forward':
                            #first write read 1..
                            #partner should be on reverse strand so needs to be reverse complemented...
                            if partner_strand == 'reverse':
                                sequence = Seq(partner_consensus)
                                partner_read.seq = str(sequence.reverse_complement())
                                partner_read.query_qualities = reverse(partner_quality)
                        ...
                        if strand == 'reverse':
                            #first write read 1..
                            #partner should be on the forward strand so does not need to be reverse complemented
                            if partner_strand == 'reverse': #it shouldn't be, but sometimes there might have been mismapping
                                sequence = Seq(partner_consensus)
                                partner_read.seq = str(sequence.reverse_complement())
                                partner_read.query_qualities = reverse(partner_quality)
...
def main():
    ...
    parser.add_argument("--outfastq", action="store", dest="outfastq", help="output SSCS fastq file", required=True)
    ...
    #Call SSCS
    SSCS_calling_BAM(sample_name, grouped_read_dict, merged_bam, threshold, min_family_size, max_N, SSCS_bam, SSCS_fastq, unpaired_SSCS_bam, out_directory, quality_threshold, version, min_mapping_qual)
